...
def optimize_decision_tree():
    ...
    print("Starting Random Forest optimization with GWO on Mushroom dataset...")
    ...
    problem = {
        "bounds": [
            FloatVar(lb=3, ub=30, name="max_depth"),              # max tree depth
            FloatVar(lb=10, ub=200, name="n_estimators"),         # number of trees
            FloatVar(lb=2, ub=20, name="min_samples_split"),      
            FloatVar(lb=0.3, ub=1.0, name="max_features")         # feature subsample fraction
        ],
        "minmax": "min",
        "obj_func": objective_function
    }

    optimizer = GWO.OriginalGWO(epoch=5, pop_size=20)
    ...
    best_params = {
        ...
        'n_estimators': int(best_position[1]),
        'min_samples_split': int(best_position[2]),
        'max_features': float(best_position[3]),
        'random_state': 42,
        'n_jobs': -1
    }
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("OPTIMIZATION COMPLETE! (Random Forest + GWO)")
    print("="*50)
